...
3. [플로우 차트 (Flow Chart)](#플로우-차트-flow-chart)
    - [플로우 차트 설명](#플로우-차트-설명)
4. [시퀀스 다이어그램](#시퀀스-다이어그램)
    ...
    - [장바구니 관련](#장바구니-관련)
    - [상품 관련](#상품-관련)
5. [Mock API 생성](#mock-api-생성)
6. [프로젝트 구조 및 실행 방법](#프로젝트-구조-및-실행-방법)
    - [프로젝트 구조](#프로젝트-구조)
...
## 기능 명세서
#### 1. 선착순 쿠폰 조회
- **설명**: 인증된 유저가 특정 선착순 쿠폰의 상태와 본인의 발급 여부를 확인할 수 있습니다.
- **주요 흐름**:
  - 유저가 쿠폰 조회 요청을 보냅니다.
  - 서버는 쿠폰의 남은 수량, 총 수량, 유저의 발급 이력을 확인하여 응답합니다.
- **기대 동작**: 유저는 쿠폰 발급 가능 여부와 상태를 빠르게 확인할 수 있습니다.
#### 2. 선착순 쿠폰 발급
- **설명**: 인증된 유저가 선착순 쿠폰을 요청하여 발급받을 수 있습니다.
- **주요 흐름**:
  - 서버는 쿠폰 잔여 수량을 확인하고, 발급 가능 여부를 판단합니다.
  - 발급 가능한 경우 쿠폰 수량을 감소시키고 유저 이력에 추가합니다.
- **기대 동작**: 발급 성공 시 유저는 쿠폰을 사용할 수 있으며, 실패 시 적절한 이유가 제공됩니다.
#### 3. 유저 쿠폰 목록 조회
- **설명**: 인증된 유저가 보유한 모든 쿠폰의 목록과 각 쿠폰의 상세 정보를 확인할 수 있습니다.
- **주요 흐름**:
  - 서버는 유저의 보유 쿠폰 목록과 각 쿠폰의 상세 정보를 조회하여 반환합니다.
- **기대 동작**: 유저는 자신의 모든 쿠폰 정보(상세 조건 포함)를 확인할 수 있습니다.
#### 4. 유저 잔액 조회
- **설명**: 인증된 유저가 자신의 현재 잔액을 확인할 수 있습니다.
- **주요 흐름**:
  - 서버는 유저의 잔액 데이터를 조회하여 반환합니다.
- **기대 동작**: 유저는 자신의 현재 잔액 정보를 정확히 확인할 수 있습니다.
#### 5. 유저 잔액 충전
- **설명**: 인증된 유저가 자신의 잔액을 요청 금액만큼 충전할 수 있습니다.
- **주요 흐름**:
  - 서버는 충전 요청을 처리하고, 잔액 데이터를 업데이트합니다.
- **기대 동작**: 충전 성공 시 잔액이 갱신되고 이력이 저장됩니다.
#### 6. 인기 상품 조회
- **설명**: 최근 특정 기간 동안 가장 많이 판매된 상품을 조회합니다.
- **주요 흐름**:
  - 캐시에 저장된 인기 상품 데이터를 확인하거나, 데이터베이스에서 최신 데이터를 집계합니다.
- **기대 동작**: 인기 상품 목록을 빠르고 정확하게 제공하며, 캐시를 활용해 성능을 최적화합니다.
#### 7. 상품 상세 조회
- **설명**: 특정 상품의 상세 정보를 유저에게 제공합니다.
- **주요 흐름**:
  - 서버는 상품의 기본 정보, 이미지, 옵션, 설명 등을 조회하여 반환합니다.
- **기대 동작**: 유저는 상품의 모든 정보를 상세히 확인할 수 있습니다.
#### 8. 장바구니 상품 추가
- **설명**: 인증된 유저가 특정 상품을 장바구니에 추가할 수 있습니다.
- **주요 흐름**:
  - 서버는 요청한 상품과 옵션의 재고를 확인한 후, 장바구니에 추가하거나 수량을 갱신합니다.
- **기대 동작**: 유저가 원하는 상품을 장바구니에 성공적으로 추가하거나, 재고 부족 메시지를 받습니다.
#### 9. 장바구니 상품 삭제
- **설명**: 유저가 장바구니에서 특정 상품을 삭제할 수 있습니다.
- **주요 흐름**:
  - 서버는 요청한 상품 정보를 확인하고 장바구니에서 해당 상품을 제거합니다.
- **기대 동작**: 유저가 선택한 상품이 장바구니에서 성공적으로 삭제됩니다.
#### 10. 장바구니 상품 주문
- **설명**: 유저가 장바구니에 담긴 상품을 주문합니다.
- **주요 흐름**:
  - 서버는 선택된 상품의 재고를 확인하고, 주문 데이터를 생성하며, 장바구니를 비웁니다.
- **기대 동작**: 주문이 성공적으로 생성되고, 결제 프로세스로 진행됩니다.
#### 11. 상품 직접 주문
- **설명**: 유저가 장바구니를 거치지 않고 특정 상품을 직접 주문할 수 있습니다.
- **주요 흐름**:
  - 서버는 요청된 상품의 재고와 쿠폰의 유효성을 확인한 후 주문 데이터를 생성합니다.
- **기대 동작**: 유저가 상품을 빠르게 주문할 수 있습니다.
#### 12. 상품 결제
- **설명**: 유저가 주문된 상품의 결제를 진행합니다.
- **주요 흐름**:
  - 서버는 주문 정보를 확인하고, 유저의 잔액에서 금액을 차감한 뒤, 재고와 결제 상태를 갱신합니다.
  - 성공 시 결제 완료 이벤트를 비동기적으로 전송합니다.
- **기대 동작**: 결제가 성공적으로 완료되거나, 잔액 부족 시 실패 메시지가 반환됩니다.
<br/><br/>
